...
class SLPagingViewSwift: UIViewController, UIScrollViewDelegate {
    ...
    var navigationSideItemsStyle: SLNavigationSideItemsStyle = .SLNavigationSideItemsStyleOnBounds
    ...
    private var indexSelected: NSInteger    = 2
    ...
    init(items: NSArray, views: NSArray, showPageControl: Bool, navBarBackground: UIColor){
        ...
        for item in items{
            if item.isKindOfClass(UIView.classForCoder()){
                ...
                let vSize: CGSize = v.isKindOfClass(UILabel.classForCoder()) ? self.getLabelSize(v as! UILabel) : v.frame.size
        ...
        if (views.count > 0){
            ...
            if controllerKeys.count == views.count {
               self.viewControllers = NSDictionary(objects: views as [AnyObject], forKeys: controllerKeys as [AnyObject])
                ...
                let exc = NSException(name: "View Controllers error", reason: "Some objects in viewControllers are not kind of UIViewController!", userInfo: nil)
                exc.raise()
            }
        }
    }
    ...
    */
    func setCurrentIndex(index: NSInteger, animated: Bool){
        // Be sure we get an existing index
        if(index < 0 || index > self.navigationBarView.subviews.count-1){
            let exc = NSException(name: "Index out of range", reason: "The index is out of range of subviews's counts!", userInfo: nil)
            exc.raise()
        }
    ...
    private func setupPagingProcess() {
        let frame: CGRect                              = CGRectMake(0, 0, SCREENSIZE.width, SCREENSIZE.height)
    ...
    private func addViews() {
        if self.viewControllers.count > 0 {
            ...
            self.viewControllers.enumerateKeysAndObjectsUsingBlock({ key, obj, stop in
                let rect: CGRect = CGRectMake(self.SCREENSIZE.width * CGFloat(i), 0, self.SCREENSIZE.width, self.SCREENSIZE.height - 50)
                let v = obj as! UIView
                v.frame = rect
    ...
    func tapOnHeader(recognizer: UITapGestureRecognizer){
        if(self.isUserInteraction){
            let key  = recognizer.view?.tag
            let view = self.viewControllers.objectForKey(NSNumber(integer: key!)) as! UIView
    ...
    func scrollViewDidScroll(scrollView: UIScrollView) {
        ...
        for obj in self.navItems {
            let v:UIView = obj as! UIView
            let distance = 100 + self.navigationSideItemsStyle.rawValue
            let vSize    = v.frame.size
            let originX  = self.getOriginX(vSize, idx: CGFloat(i), distance: CGFloat(distance), xOffset: CGFloat(xOffset))
            v.frame      = CGRectMake(originX, 8, vSize.width, vSize.height)
            i++
        }
